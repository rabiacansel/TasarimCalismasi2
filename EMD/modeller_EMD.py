...
def BİLSTM():
    ...
    callbacks = [
        ModelCheckpoint('BiLSTM_EMD_best_model.keras', monitor='val_loss', save_best_only=True, mode='min', verbose=1),
    ...
    BiLSTM_EMD_best_model = load_model('BiLSTM_EMD_best_model.keras')
    loss, accuracy = BiLSTM_EMD_best_model.evaluate(X_test_bilstm, y_test_bilstm)
    model.save("GRU_EMD_best_model.keras")
    ...
    y_pred = np.argmax(BiLSTM_EMD_best_model.predict(X_test_bilstm), axis=1)
...
def GRU_Model():
    ...
    callbacks = [
        ModelCheckpoint('GRU_EMD_best_model.keras', monitor='val_loss', save_best_only=True, mode='min', verbose=1)
    ]
    
    history = model.fit(X_train_gru, y_train_gru, epochs=100, batch_size=100, validation_split=0.1, callbacks=callbacks)
    GRU_EMD_best_model = load_model('GRU_EMD_best_model.keras')
    loss, accuracy = GRU_EMD_best_model.evaluate(X_test_gru, y_test_gru)
    model.save("GRU_EMD_best_model.keras")
    ...
    y_pred = np.argmax(GRU_EMD_best_model.predict(X_test_gru), axis=1)
